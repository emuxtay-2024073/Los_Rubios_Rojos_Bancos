  ...
  useEffect(() => {
    if (isOpen) {
      if (field) {
        ...
        setPreview(resolveCloudinaryImageUrl(field.photo));
      } else {
        reset({
          fieldName: '',
          fieldType: '',
    ...
    <div className='fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50 px-3 sm:px-4'>
      ...
      <div className='bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[92vh] flex flex-col overflow-hidden'>
        ...
        <div
          ...
          style={{
            background: 'linear-gradient(90deg, #0A2472 0%, #1E40AF 100%)',
          }}
        >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='p-5 sm:p-6 space-y-5 overflow-y-auto'>
          ...
          <div className='flex flex-col-reverse sm:flex-row sm:justify-end gap-3 pt-4 border-t border-gray-400/80'>
            ...
            <button
              type='submit'
              className='w-full sm:w-auto px-5 py-2 rounded-lg bg-main-blue hover:opacity-90 text-white font-medium transition shadow'
            >
