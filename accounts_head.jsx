  ...
  const handleActionSubmit = async (event) => {
    ...
    try {
      ...
      if (actionType === 'deposit') {
        ...
        showSuccess('Dep├│sito realizado');
      } else {
      ...
      showError(error?.response?.data?.message || 'No se pudo procesar la operaci├│n');
    }
  };
  ...
  return (
    <div className='space-y-8'>
      <div className='flex flex-col gap-4 md:flex-row md:justify-between md:items-end'>
        <div>
          <p className='text-sm text-gray-500'>Gesti├│n de cuentas</p>
          <h1 className='text-3xl font-bold text-main-blue'>Cuentas</h1>
          {isAdmin && (
            <p className='mt-2 text-sm text-gray-500'>Vista de s├│lo lectura para administradores. Aqu├¡ puedes ver datos y filtrar cuentas, pero no crear ni editar operaciones.</p>
          )}
        </div>
        {!isAdmin && (
          <button
      ...
      <div className='flex flex-col gap-4 md:flex-row md:items-center'>
        <input
          ...
          placeholder='Buscar por n├║mero, tipo o saldo'
            ...
            <div className='mt-6 space-y-4'>
              ...
              <div className='rounded-3xl border border-gray-200 bg-white p-4'>
                ...
                {accountMovements.length === 0 ? (
                  <p className='text-sm text-gray-500'>No se encontraron movimientos de dep├│sito o retiro para esta cuenta.</p>
                ) : (
      ...
      {actionModalOpen && activeAccount && (
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4'>
          <div className='w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl'>
            <div className='flex items-center justify-between gap-4'>
              <div>
                <p className='text-sm text-gray-500'>{actionType === 'deposit' ? 'Dep├│sito' : 'Retiro'}</p>
